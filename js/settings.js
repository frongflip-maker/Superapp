...
function NotificationSettings(props) {
    const reminderMinutes = (props && props.reminderMinutes) ?? 15;
    const setReminderMinutes = (props && props.setReminderMinutes) || (() => { });
    const remindersEnabled = (props && props.remindersEnabled) !== false;
    const setRemindersEnabled = (props && props.setRemindersEnabled) || (() => { });
    ...
    return (React.createElement("div", { className: "mb-4" },
        ...
        subscribed ? (React.createElement("div", { className: "space-y-2" },
            ...
            React.createElement("div", { className: "p-2.5 rounded-xl", style: { background: T.surf2, border: `1px solid ${T.border}` } },
                React.createElement("button", { onClick: () => setRemindersEnabled(!remindersEnabled), className: "w-full flex items-center justify-between active:scale-[.99]" },
                    React.createElement("span", { className: "text-xs font-semibold", style: { color: remindersEnabled ? T.text : T.sub } }, "เตือน event ที่สร้างไว้อัตโนมัติ"),
                    React.createElement("span", { className: "w-9 h-5 rounded-full flex items-center px-0.5 transition shrink-0", style: { background: remindersEnabled ? T.purple : T.border, justifyContent: remindersEnabled ? "flex-end" : "flex-start" } },
                        React.createElement("span", { className: "w-4 h-4 rounded-full", style: { background: "#fff" } }))),
                remindersEnabled && React.createElement("div", { className: "mt-2.5" },
                    React.createElement("div", { className: "text-[11px] mb-1.5", style: { color: T.muted } }, "เตือนล่วงหน้าก่อนถึงเวลา event"),
                    React.createElement("div", { className: "flex gap-1.5 flex-wrap" }, [5, 10, 15, 30, 60].map(m => {
                        const on = reminderMinutes === m;
                        return React.createElement("button", { key: m, onClick: () => setReminderMinutes(m), className: "px-2.5 py-1.5 rounded-lg text-xs font-bold", style: { background: on ? T.purple : T.surf, color: on ? "#fff" : T.sub, border: `1px solid ${on ? T.purple : T.border}` } }, m >= 60 ? "1 ชม." : m + " นาที");
                    })),
                    React.createElement("div", { className: "text-[10px] mt-2", style: { color: T.muted } }, "ใช้กับทุก event ที่ตั้งเวลาไว้ · event ที่ตั้งซ้ำ (รายวัน/สัปดาห์) จะเตือนทุกครั้ง")))) ) : (React.createElement("button", { onClick: enable, disabled: busy, className: "w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 active:scale-[.98]", style: { background: T.purple, color: "#fff" } },
...
function SettingsBody(props) {
    ...
    return (React.createElement(React.Fragment, null,
                    React.createElement("button", { onClick: () => Auth.signOut(), className: "w-full py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 active:scale-[.98] mb-3", style: { background: T.surf2, color: T.red, border: `1px solid ${hex(T.red, .4)}` } },
                        ...
                        " \u0E2D\u0E2D\u0E01\u0E08\u0E32\u0E01\u0E23\u0E30\u0E1A\u0E1A"),
                    React.createElement(NotificationSettings, { reminderMinutes: props.reminderMinutes, setReminderMinutes: props.setReminderMinutes, remindersEnabled: props.remindersEnabled, setRemindersEnabled: props.setRemindersEnabled }),
