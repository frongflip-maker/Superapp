...
function ReminderCategory({ icon: Icon, color, label, hint, cat, prefs, setCat }) {
    const p = prefs[cat];
    const on = p.on;
    const isMoney = cat === "budget";
    const opts = isMoney ? [[0, "ทันที"], [1, "หลัง 1 วัน"], [3, "หลัง 3 วัน"]] : [[5, "5 นาที"], [10, "10 นาที"], [15, "15 นาที"], [30, "30 นาที"], [60, "1 ชม."]];
    const cur = isMoney ? (Number(p.delayDays) || 0) : (Number(p.min) || 15);
    return React.createElement("div", { className: "p-2.5 rounded-xl", style: { background: T.surf2, border: `1px solid ${on ? hex(color, .4) : T.border}` } },
        React.createElement("button", { onClick: () => setCat(cat, { on: !on }), className: "w-full flex items-center gap-2 active:scale-[.99]" },
            React.createElement("span", { className: "w-7 h-7 rounded-lg flex items-center justify-center shrink-0", style: { background: hex(color, .16) } }, React.createElement(Icon, { size: 14, style: { color } })),
            React.createElement("div", { className: "flex-1 text-left min-w-0" },
                React.createElement("div", { className: "text-xs font-bold", style: { color: on ? T.text : T.sub } }, label),
                React.createElement("div", { className: "text-[10px] truncate", style: { color: T.muted } }, hint)),
            React.createElement("span", { className: "w-9 h-5 rounded-full flex items-center px-0.5 transition shrink-0", style: { background: on ? color : T.border, justifyContent: on ? "flex-end" : "flex-start" } },
                React.createElement("span", { className: "w-4 h-4 rounded-full", style: { background: "#fff" } }))),
        on && React.createElement("div", { className: "mt-2.5" },
            React.createElement("div", { className: "text-[10px] mb-1.5", style: { color: T.muted } }, isMoney ? "เตือนเมื่อไหร่" : "เตือนล่วงหน้าก่อนถึงเวลา"),
            React.createElement("div", { className: "flex gap-1.5 flex-wrap" }, opts.map(([v, lbl]) => {
                const sel = cur === v;
                return React.createElement("button", { key: v, onClick: () => setCat(cat, isMoney ? { delayDays: v } : { min: v }), className: "px-2.5 py-1.5 rounded-lg text-[11px] font-bold", style: { background: sel ? color : T.surf, color: sel ? "#000" : T.sub, border: `1px solid ${sel ? color : T.border}` } }, lbl);
            }))));
}
function NotificationSettings(props) {
    const prefs = Notify.normPrefs(props && props.notifPrefs);
    const setNotifPrefs = (props && props.setNotifPrefs) || (() => { });
    const setCat = (cat, patch) => setNotifPrefs(prev => { const np = Notify.normPrefs(prev); np[cat] = { ...np[cat], ...patch }; return np; });
    const [perm, setPerm] = useState("default");
    const [subscribed, setSubscribed] = useState(false);
    const [busy, setBusy] = useState(false);
    const [msg, setMsg] = useState("");
    const refresh = async () => {
        setPerm(await Push.permission());
    ...
    const enable = async () => {
        ...
        try {
            ...
            setMsg((e && e.message) || "เปิดแจ้งเตือนไม่สำเร็จ");
        }
        setBusy(false);
    };
    const disable = async () => {
        ...
        try {
            ...
            setMsg((e && e.message) || "ปิดแจ้งเตือนไม่สำเร็จ");
        }
        setBusy(false);
    };
    const test = async () => {
        ...
        try {
            ...
            setMsg(r && r.sent ? `ส่งแจ้งเตือนทดสอบแล้ว (${r.sent}/${r.total})` : "ส่งไม่สำเร็จ — เช็คว่าตั้งค่า Edge Function เสร็จหรือยัง");
        }
        catch (e) {
            setMsg((e && e.message) || "ส่งไม่สำเร็จ");
        }
        setBusy(false);
    };
    if (!Push.supported()) {
        return (React.createElement("div", { className: "mb-4" },
            React.createElement(Label, null, "การแจ้งเตือน"),
            React.createElement("div", { className: "text-[11px] p-3 rounded-xl", style: { background: T.surf2, color: T.muted, border: `1px solid ${T.border}` } }, "อุปกรณ์/เบราว์เซอร์นี้ไม่รองรับ — บน iPhone ต้องกด Share → Add to Home Screen แล้วเปิดแอปจากหน้าจอ Home ก่อนถึงจะเปิดแจ้งเตือนได้")));
    }
    return (React.createElement("div", { className: "mb-4" },
        React.createElement(Label, null, "การแจ้งเตือน"),
        subscribed ? (React.createElement("div", { className: "space-y-2" },
            React.createElement("div", { className: "flex items-center justify-between p-2.5 rounded-xl", style: { background: T.surf2, border: `1px solid ${hex(T.green, .4)}` } },
                React.createElement("div", { className: "flex items-center gap-2" },
                    React.createElement(Bell, { size: 15, style: { color: T.green } }),
                    React.createElement("span", { className: "text-xs font-semibold" }, "เปิดแจ้งเตือนอยู่")),
                React.createElement("button", { onClick: disable, disabled: busy, "aria-label": "ปิดแจ้งเตือน", className: "text-[11px] font-semibold px-2 py-1 rounded-lg", style: { color: T.red } }, "ปิด")),
            React.createElement("div", { className: "text-[11px] font-semibold mt-1 mb-0.5", style: { color: T.sub } }, "เลือกหัวข้อที่จะให้เตือน"),
            React.createElement(ReminderCategory, { icon: CalendarDays, color: T.purple, label: "Event ทั่วไป", hint: "งาน/นัด/คอนเทนต์ ที่ตั้งเวลาไว้", cat: "event", prefs, setCat }),
            React.createElement(ReminderCategory, { icon: Music, color: T.violet, label: "ห้องซ้อม", hint: "การจอง/คิวห้องซ้อม", cat: "studio", prefs, setCat }),
            React.createElement(ReminderCategory, { icon: Dumbbell, color: T.green, label: "ออกกำลังกาย", hint: "ตารางฟิตเนส/กีฬา", cat: "fitness", prefs, setCat }),
            React.createElement(ReminderCategory, { icon: Wallet, color: T.orange, label: "เงินค้างรับ", hint: "งาน/ห้องซ้อม ที่ยังไม่ได้รับเงิน", cat: "budget", prefs, setCat }),
            React.createElement("button", { onClick: test, disabled: busy, className: "w-full py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 active:scale-[.98]", style: { background: T.surf2, color: T.text, border: `1px solid ${T.border}` } }, busy ? "กำลังส่ง…" : "ส่งแจ้งเตือนทดสอบ"),
            React.createElement("div", { className: "text-[10px]", style: { color: T.muted } }, "event ที่ตั้งซ้ำ (รายวัน/สัปดาห์) จะเตือนทุกครั้งอัตโนมัติ · เปลี่ยนแล้วบันทึกเอง"))) : (React.createElement("button", { onClick: enable, disabled: busy, className: "w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 active:scale-[.98]", style: { background: T.purple, color: "#fff" } },
            React.createElement(Bell, { size: 16 }),
            busy ? "กำลังเปิด…" : "เปิดการแจ้งเตือน")),
        msg && React.createElement("div", { className: "text-[11px] mt-1.5 px-1", style: { color: msg.includes("ไม่สำเร็จ") ? T.red : T.muted } }, msg)));
}
function SettingsBody(props) {
    ...
    return (React.createElement(React.Fragment, null,
                    React.createElement("button", { onClick: () => Auth.signOut(), className: "w-full py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 active:scale-[.98] mb-3", style: { background: T.surf2, color: T.red, border: `1px solid ${hex(T.red, .4)}` } },
                        React.createElement(LogOut, { size: 15 }),
                        " \u0E2D\u0E2D\u0E01\u0E08\u0E32\u0E01\u0E23\u0E30\u0E1A\u0E1A"),
                    React.createElement(NotificationSettings, { notifPrefs: props.notifPrefs, setNotifPrefs: props.setNotifPrefs }),
...
function SettingsSheet(props) {
    return React.createElement(Sheet, { onClose: () => { props.setSettingsOpen(false); props.setDataMsg(""); props.setConfirmClear(false); }, title: "\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32 & \u0E41\u0E1A\u0E47\u0E01\u0E2D\u0E31\u0E1E" }, React.createElement(SettingsBody, props));
}
function SettingsPage(props) {
    return React.createElement("div", { className: "px-4 pt-6" },
        React.createElement("h1", { className: "text-xl font-extrabold mb-4", style: { color: T.text } }, "\u0E15\u0E31\u0E49\u0E07\u0E04\u0E48\u0E32"),
        React.createElement(SettingsBody, props));
}
