...
try {
    function App() {
        ...
        const [notifPrefs, setNotifPrefs] = useState(() => Notify.defaultPrefs());
        ...
        const applySnapshot = (s) => {
            ...
            if (s.notifPrefs)
                setNotifPrefs(Notify.normPrefs(s.notifPrefs));
            else if (typeof s.reminderMinutes === "number")
                setNotifPrefs(Notify.normPrefs({ event: { on: s.remindersEnabled !== false, min: s.reminderMinutes } }));
        };
        useEffect(() => { const s = migrate(storage.load()); if (s)
            applySnapshot(s); setLoaded(true); }, []);
        useEffect(() => { if (loaded)
            storage.save(buildSnapshot()); }, [loaded, events, bookings, loyaltyBase, transactions, capital, workouts, nutrition, nutriTargets, exPresets, workoutPresets, accounts, txnCategories, fixedCosts, expensePresets, fitnessSplit, fitnessStartDay, fitnessConfirmed, driveConnected, driveFileId, autoSync, notifPrefs]);
        ...
        useEffect(() => {
            ...
            reminderTimer.current = setTimeout(() => { Notify.syncReminders(events, bookings, notifPrefs); }, 2500);
            return () => { if (reminderTimer.current)
                clearTimeout(reminderTimer.current); };
        }, [loaded, events, bookings, notifPrefs]);
        useEffect(() => {
        ...
        const shared = { T, sel, setSel, events: allEvents, todayEvents, bookings,
            ...
            toggleEvent, toggleEventPaid, delEvent, delEventSeries, editEvent, editEventSeries, addEvent, addBooking, markBookingPaid, openSettings: () => setTab("settings") };
        ...
        const alertCount = useMemo(() => Notify.computeAlerts(events, bookings, notifPrefs).length, [events, bookings, notifPrefs]);
        const NAV = [["today", Home, "Today"], ["calendar", CalendarDays, "Calendar"], ["studio", Music, "Studio"], ["budget", Wallet, "Budget"], ["fitness", Dumbbell, "Fitness"], ["dashboard", BarChart3, "Dashboard"], ["settings", Settings, "Settings"]];
        return (React.createElement("div", { style: { background: T.bg, color: T.text, fontFamily: "system-ui,-apple-system,sans-serif" }, className: "min-h-screen w-full flex justify-center" },
            ...
            React.createElement("div", { className: "w-full max-w-md flex flex-col min-h-screen relative" },
                React.createElement("button", { onClick: () => setTab("settings"), className: "absolute left-1/2 -translate-x-1/2 z-40 flex items-center gap-1.5 px-2.5 py-1.5 rounded-full active:scale-95", style: { top: "calc(12px + env(safe-area-inset-top))", background: hex(T.surf, .9), border: `1px solid ${driveToken ? (syncState === "error" ? T.red : hex(T.green, .5)) : T.border}`, backdropFilter: "blur(6px)" } },
                ...
                React.createElement("div", { className: "absolute z-40", style: { top: "calc(10px + env(safe-area-inset-top))", right: "16px" } },
                    React.createElement(BellButton, { count: alertCount, onClick: () => setNotifOpen(true) })),
                React.createElement("main", { className: "flex-1 pb-24" },
                    ...
                    tab === "settings" && React.createElement(SettingsPage, settingsProps)),
                    ...
                    React.createElement("div", { className: "w-full max-w-md px-3 pb-3 pt-2", style: { background: `linear-gradient(to top, ${T.bg}, ${hex(T.bg, .85)} 70%, transparent)` } },
                        React.createElement("div", { className: "flex items-center justify-around rounded-2xl py-1.5", style: { background: hex(T.surf, .9), border: `1px solid ${T.border}`, backdropFilter: "blur(8px)" } }, NAV.map(([id, Icon, label]) => {
                            ...
                            return (React.createElement("button", { key: id, onClick: () => setTab(id), "aria-label": label, className: "flex flex-col items-center gap-0.5 px-1.5 py-1.5 rounded-xl transition active:scale-90" },
                                ...
                                notifOpen && React.createElement(NotificationCenter, { events, bookings, notifPrefs, onClose: () => setNotifOpen(false) }))));
    }
    function Root() {
        const [session, setSession] = useState(undefined);
        useEffect(() => {
            let cancelled = false;
            Auth.getSession().then(s => { if (!cancelled)
                setSession(s); });
            const unsub = Auth.onChange(s => setSession(s));
            return () => { cancelled = true; unsub(); };
        }, []);
        if (session === undefined) {
            return React.createElement("div", { className: "min-h-screen w-full flex items-center justify-center", style: { background: T.bg, color: T.muted } }, "กำลังโหลด…");
        }
        if (!session) {
