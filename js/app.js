...
try {
    function App() {
        ...
        const applySnapshot = (s) => {
            ...
            if (typeof s.remindersEnabled === "boolean")
                setRemindersEnabled(s.remindersEnabled);
        };
        useEffect(() => { const s = migrate(storage.load()); if (s)
            applySnapshot(s); setLoaded(true); }, []);
        useEffect(() => { if (loaded)
            storage.save(buildSnapshot()); }, [loaded, events, bookings, loyaltyBase, transactions, capital, workouts, nutrition, nutriTargets, exPresets, workoutPresets, accounts, txnCategories, fixedCosts, expensePresets, fitnessSplit, fitnessStartDay, fitnessConfirmed, driveConnected, driveFileId, autoSync, reminderMinutes, remindersEnabled]);
        ...
        useEffect(() => {
            ...
            reminderTimer.current = setTimeout(() => { Notify.syncReminders(events, reminderMinutes, remindersEnabled); }, 2500);
            return () => { if (reminderTimer.current)
                clearTimeout(reminderTimer.current); };
        }, [loaded, events, reminderMinutes, remindersEnabled]);
        useEffect(() => {
        ...
        const settingsProps = { driveToken, driveEmail, connectDrive, disconnectDrive,
            ...
            reminderMinutes, setReminderMinutes, remindersEnabled, setRemindersEnabled };
        const alertCount = useMemo(() => Notify.computeAlerts(events, bookings).length, [events, bookings]);
                    ...
                    React.createElement("div", { className: "w-full max-w-md px-3 pb-3 pt-2", style: { background: `linear-gradient(to top, ${T.bg}, ${hex(T.bg, .85)} 70%, transparent)` } },
                        React.createElement("div", { className: "flex items-center justify-around rounded-2xl py-1.5", style: { background: hex(T.surf, .9), border: `1px solid ${T.border}`, backdropFilter: "blur(8px)" } }, NAV.map(([id, Icon, label]) => {
                            ...
                            return (React.createElement("button", { key: id, onClick: () => setTab(id), "aria-label": label, className: "flex flex-col items-center gap-0.5 px-1.5 py-1.5 rounded-xl transition active:scale-90" },
                                ...
                                notifOpen && React.createElement(NotificationCenter, { events, bookings, onClose: () => setNotifOpen(false) }))));
    }
    function Root() {
